...
for i = 1:target_num
    ...
    for ii = 1:trial_num
        x = Base_FB_Target{i}(ii).Gaze_X;
        y = Base_FB_Target{i}(ii).Gaze_Y;
        t = Base_FB_Target{i}(ii).Gaze_TimeStamp;
        trial = Base_FB_Target{i}(ii).TRIAL(1).TRIAL_NUM * ones(size(x));
        target = str2num(Base_FB_Target{i}(ii).FILE_NAME(4:5)) * ones(size(x));
        Gaze_Baseline_FB = [Gaze_Baseline_FB; trial target x y t];
    end
    
    for ii = 1:trial_num
        x = Base_FB_Target{i}(ii).Right_HandX;
        y = Base_FB_Target{i}(ii).Right_HandY;
        trial = Base_FB_Target{i}(ii).TRIAL(1).TRIAL_NUM * ones(size(x));
        target = str2num(Base_FB_Target{i}(ii).FILE_NAME(4:5)) * ones(size(x));
        Traj_Baseline_FB = [Traj_Baseline_FB; trial target x y];
    end       
    
    % Target Index    
    target_temp = []
    for ii = 1:trial_num
        Base_FB_Target_num_temp = Base_FB_Target{i}(ii).FILE_NAME;
        Base_FB_Target_num_temp = Base_FB_Target_num_temp(4:5);
        Base_FB_Target_num_temp = str2num(Base_FB_Target_num_temp);
        target_temp = [target_temp; Base_FB_Target_num_temp];
    end
    ...
    trial_temp = []
    for ii = 1:trial_num
        Base_FB_Trial_idx_temp = Base_FB_Target{i}(ii).TRIAL(1).TRIAL_NUM
        trial_temp = [trial_temp; Base_FB_Trial_idx_temp];
    end
...
% NOTE: Gaze_Baseline_FB is created at the top
